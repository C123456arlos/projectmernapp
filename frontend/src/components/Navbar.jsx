  ...
  return (
    <header className="fixed top-0 left-0 w-full bg-gray-900 bg-opacity-90 backdrop-blur-md shadow-lg z-40 transition-all duration-300 border-b border-emerald-800">
      <div className="container mx-auto px-4 py-3">
        ...
        <Link to='/' className="text-2xl font-bold text-emerald-400 items-center space-x-2 flex">mern commerce</Link>
...
{  cart.length>0 &&           <span className="absolute -top-2 -left-2 bg-emerald-500 text-white rounded-full px-2 py-0.5 text-xs
              group-hover:bg-emerald-400 transtion duration-300 ease-in-out">{cart.length}</span>}
            </Link>
          )}
            {isAdmin && user && (<Link to={'/secret-dashboard'}
              className="bg-emerald-700 hover:bg-emerald-600 text-white px-3 py-1 rounded-md font-medium transition duration-300 ease-in-out flex items-center">
            <Lock className="inline-block mr-1" size={18}></Lock>
            <span className="hidden sm:inline">dashboard</span>
          </Link>)}
          {user ? (<button className="bg-gray-700 hover:bg-gray-600 text-white py-2 px-4 rounded-md flex items-center transition duration-300 ease-in-out" onClick={logout}>
            ...
            <span className="hidden sm:inline ml-2">logout</span>
              ...
              <>
                <Link to={'/signup'} className="bg-emerald-600 hover:bg-emerald-700 text-white py-2 px-4 rounded-md flex items-center transition duration-300 ease-in-out">
                  <UserPlus className="mr-2" size={18}></UserPlus>
                  sign up</Link>
